...
```bash
dk bash  # starts bash inside "todolistdjv3" container
...
- `dkup` uses docker-compose to start 3 containers: postgres, nginx, and todolistdjv3.
...
- Once `dkup` is running, `dk <command>` will run `<command>` inside the `todolistdjv3` container. So `dk bash` will get you "logged in" as root inside that container. Once inside, you need to run Django's `manage.py` commands to initialize the database properly.
- The todolistdjv3 container runs 3 services:
- django on port 8000
...
```bash
mkvirtualenv todolistdjv3 -p python3  # creates a python3 virtualenv
